...
inline def registerTest(testText: String, testTags: Tag*)(testFun: => Any /* Assertion */)(implicit pos: source.Position): Unit = {  // Note: we can't remove the implicit pos here because it is the signature of registerTest in TestRegistration.
...
inline def registerIgnoredTest(testText: String, testTags: Tag*)(testFun: => Any /* Assertion */)(implicit pos: source.Position): Unit = {  // Note: we can't remove the implicit pos here because it is the signature of registerTest in TestRegistration.
  ...
  protected final class ResultOfTaggedAsInvocationOnString(specText: String, tags: List[Tag]) {
     ...
     * </p>
     */
inline def in(testFun: => Any /* Assertion */): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerTestToRun(specText, tags, "in", () => testFun, pos)}) } 
}
     ...
     * </p>
     */
inline def is(testFun: => PendingStatement): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerTestToRun(specText, tags, "is", () => { testFun; succeed }, pos)}) } 
}
     ...
     * </p>
     */
inline def ignore(testFun: => Any /* Assertion */): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerTestToIgnore(specText, tags, "ignore", () => testFun, pos)}) } 
}
  }       
  ...
  protected final class WordSpecStringWrapper(string: String) {
     ...
     * </p>
     */
inline def in(f: => Any /* Assertion */): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerTestToRun(string, List(), "in", () => f, pos)}) } 
}
     ...
     * </p>
     */
inline def ignore(f: => Any /* Assertion */): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerTestToIgnore(string, List(), "ignore", () => f, pos)}) } 
}
     ...
     * </p>
     */
inline def is(f: => PendingStatement): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerTestToRun(string, List(), "is", () => { f; succeed }, pos)}) } 
}
     ...
     * </p>
     */
inline def when(f: => Unit): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerBranch(string, Some("when"), "when", "when", 4, -2, pos, () => f)}) } 
}
     ...
     * </p>
     */
inline def when(resultOfAfterWordApplication: ResultOfAfterWordApplication): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerBranch(string, Some("when " + resultOfAfterWordApplication.text), "when", "when", 4, -2, pos, resultOfAfterWordApplication.f)}) } 
}
     ...
     * </p>
     */
inline def that(f: => Unit): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerBranch(string.trim + " that", None, "that", "that", 4, -2, pos, () => f)}) } 
}
     ...
     * </p>
     */
inline def which(f: => Unit): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerBranch(string.trim + " which", None, "which", "which", 4, -2, pos, () => f)}) } 
}
     ...
     * </p>
     */
inline def that(resultOfAfterWordApplication: ResultOfAfterWordApplication): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerBranch(string.trim + " that " + resultOfAfterWordApplication.text.trim, None, "that", "that", 4, -2, pos, resultOfAfterWordApplication.f)}) } 
}
     ...
     * </p>
     */
inline def which(resultOfAfterWordApplication: ResultOfAfterWordApplication): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerBranch(string.trim + " which " + resultOfAfterWordApplication.text.trim, None, "which", "which", 4, -2, pos, resultOfAfterWordApplication.f)}) } 
}
  }
  ...
  protected final class ItWord {
     ...
     * </p>
     */
inline def should(right: => Unit): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => shouldImpl(right, pos) }) } 
}
     ...
     * </p>
     */
inline def must(right: => Unit): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => mustImpl(right, pos) }) } 
}
     ...
     * </p>
     */
inline def can(right: => Unit): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => canImpl(right, pos) }) } 
}
     ...
     * </p>
     */
inline def when(right: => Unit): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => whenImpl(right, pos) }) } 
}
  }
...
inline def should(right: => Unit): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => shouldImpl(right, pos) }) } 
     ...
     */
inline def must(right: => Unit): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerShorthandBranch(Some("must"), Resources.theyMustAppearAfterTopLevelSubject, "must", stackDepth, -2, pos, () => right)}) } 
}
     ...
     */
inline def can(right: => Unit): Unit = {
  ${ source.Position.withPosition[Unit]('{(pos: source.Position) => registerShorthandBranch(Some("can"), Resources.theyMustAppearAfterTopLevelSubject, "can", stackDepth, -2, pos, () => right)}) } 
}
